...
7.  Generate a symbolink by running `php artisan storage:link`.
8.  Run the database migrations using `php artisan migrate`.
9.  Run the database seeders using `php artisan seed`.
10. Start the development server using `php artisan serve`.
11. Access the CarRental website and administration panel by visiting `http://localhost:8000` and `http://localhost:8000/admin` respectively.
