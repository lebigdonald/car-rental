            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <p><strong>Date de début:</strong> {{ Carbon::parse($rent->start_date)->format('d/m/Y') }}
                            </p>
                            <p><strong>Date de fin:</strong> {{ Carbon::parse($rent->end_date)->format('d/m/Y') }}</p>
                            ...
                            @if($rent->payement_status == 'En Attente')
                                <div class="mt-3">
                                    <h5>Actions</h5>
                                    <form action="{{ route('admin.rent.approve', $rent->id) }}" method="POST"
                                          class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-success">Approuver (Marquer comme Payé)
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.rent.reject', $rent->id) }}" method="POST"
                                          class="d-inline">
                                        @csrf
                            ...
                            @elseif($rent->payement_status == 'Payé')
                                <div class="mt-3">
                                    <a href="{{ route('admin.rent.invoice', ['id' => $rent->id]) }}"
                                       class="btn btn-primary" target="_blank">Voir la facture</a>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="col-lg-6">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <a href="{{ route('admin.user.show', $rent->user->id) }}" class="btn btn-primary btn-sm">Voir
                                le profil complet</a>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4">
                                    <img
                                        src="{{ str_starts_with($rent->car->image_url, 'http') ? $rent->car->image_url : Storage::url($rent->car->image_url) }}"
                                ...
                                <div class="col-md-8">
                                    ...
                                    <a href="{{ route('admin.car.show', $rent->car->id) }}"
                                       class="btn btn-primary btn-sm">Voir les détails de la voiture</a>
                                </div>
                            </div>
                        </div>
                    </div>
